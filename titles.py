...
_SWAP = re.compile(
    ...
    r"|대신\s*\S+|말고\s*\S+|아니라\s*\S+|에서\s*\S+(?:으?로)\s*(?:바꾸|갈아|옮)"
)
...
KNOWN = [
    ("30년째 같은 피부라는 58세 여배우의 아침 습관", 36),
    ...
    ('"엄격한 식단 안 한다는" 34세 배우, 그런데 피부는 도자기라는데', 22),
    ...
    ("166cm에 47kg이라는 몸무게.jpg", -28),
    # 같은 소재를 인물·모순·길이로 살린 쪽. 키·몸무게 병기는 피했다(저체중 가드).
    ("47kg인데 야식은 끊은 적 없다는 40대 여배우.jpg", 87),
    ...
    ('18년 전 "소지섭이 같이 죽자고 했던" 여배우의 그 장면.jpg', 68),
    # 같은 소재를 앞 20자에 숫자·모순으로 채운 쪽. 54점 차로 확실히 앞선다.
    ("48kg인데 식단표가 없다는 34세 배우, 대신 새벽마다 한강을 뛴다", 131),
    ('"남편과 어제도 키스했다는.." 55세 여배우의 동안 피부 비결', 60),
    # 저체중 스펙 가드는 뷰티판에서도 그대로 살아 있다 (-15).
    ('"165cm 43kg" 유지한다는 62세 여배우의 공항패션', 43),
    ("명품 앰버서더인데 시장 옷 입는다는 40대 여배우", 93),
    # 의혹 제기·실명 노출은 즉시 탈락.
